...
export async function runMigrations(pool: pg.Pool): Promise<MigrationResult> {
  ...
  // to read applied versions. Both are idempotent.
  await pool.query(`
  ...
  const appliedResult = await pool.query<{ version: string }>(
    "SELECT version FROM ingestion.schema_migrations ORDER BY version"
  );
  ...
  for (const filename of migrationFiles) {
    ...
    try {
      ...
      await client.query(
        "INSERT INTO ingestion.schema_migrations (version) VALUES ($1)",
        [version]
      );
