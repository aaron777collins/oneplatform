...
export interface SyncJobPayload {
  ...
  /** UUID identifying this sync job in the sync_state table. Generated by
   *  triggerSync so the worker can reference it without relying on BullMQ's
   *  internal numeric job.id (which is NOT a UUID). */
  syncJobId: string;
...
// Built-in plugin IDs that are handled directly without the execution service sandbox.
const BUILTIN_CSV_PLUGIN_IDS = new Set(["connector-csv", "oneplatform.csv"]);

// ---------------------------------------------------------------------------
// fetchBuiltInCsvBatch — fetches and parses a remote CSV file without the
// execution sandbox. Used for connector-csv / oneplatform.csv plugin IDs only.
// Returns null when the pluginId is not a recognised built-in, so the caller
// falls through to the execution service path.
// ---------------------------------------------------------------------------

async function fetchBuiltInCsvBatch(
  pluginId: string,
  config: Record<string, unknown>,
): Promise<FetchBatchResponse | null> {
  if (!BUILTIN_CSV_PLUGIN_IDS.has(pluginId)) {
    return null;
  }

  const url = config["url"];
  if (typeof url !== "string" || url.trim() === "") {
    throw new Error("CSV connector config.url is required");
  }
  if (!url.startsWith("https://") && !url.startsWith("http://")) {
    throw new Error("CSV connector config.url must be an http:// or https:// URL");
  }
  ...
  const records: DataRecord[] = dataLines.map((line, rowIndex) => {
    const values = line.split(delimiter);
    const data: Record<string, string> = {};
    for (let i = 0; i < headers.length; i++) {
      data[headers[i] ?? `col${i + 1}`] = (values[i] ?? "").trim();
    }
    return { sourceId: String(rowIndex), data };
  });
...
export function createSyncService(deps: SyncServiceDeps): SyncService {
  ...
  async function triggerSync(
    ...
    // A "running" sync_state that has no corresponding active/waiting BullMQ job
    // is stale (e.g. the worker crashed or failed before updating the DB). In
    // that case we allow re-trigger rather than leaving the connector permanently
    // locked. The watchdog also resets these after 15 minutes, but this gives
    // immediate recovery.
    if (!options.force) {
      const syncState = await syncStateRepo.findByConnectorId(connectorId);
      if (syncState?.status === "running") {
        const activeJobs = await syncQueue.getJobs(["active", "waiting"]);
        const hasLiveJob = activeJobs.some((j) => j.data.connectorId === connectorId);
        if (hasLiveJob) {
        ...
        // No live BullMQ job — stale sync_state. Reset it to failed so the
        // connector isn't permanently locked, then allow the new trigger below.
        await syncStateRepo.updateStatus(connectorId, "failed", {
          last_error: "Previous sync interrupted — reset by trigger guard",
        });
        logger.warn("Stale sync_state reset by trigger guard", { connectorId });
      }
    }

    const syncMode = options.mode ?? connector.sync_mode;

    // Generate a proper UUID for this sync job. BullMQ's job.id is a numeric
    // auto-incrementing string ("1", "2", "3") which is NOT a valid UUID and
    // cannot be stored in the sync_state.last_sync_job_id column. The UUID is
    // included in the job payload so the worker reads it from job.data.syncJobId.
    ...
    logger.info("Sync triggered", { connectorId, tenantId, syncMode, syncJobId });

    return { syncJobId, status: "queued", estimatedStartMs: 0 };
  }
  ...
  async function listSyncs(
    ...
    const filtered = jobs
      ...
      .map((job): SyncJobSummary => {
        ...
        return {
          syncJobId: job.data.syncJobId ?? job.id ?? "",
          connectorId,
          status: bullState,
  ...
  async function processSyncJob(job: Job<SyncJobPayload>): Promise<void> {
    ...
    // Use the UUID from job.data, NOT BullMQ's numeric job.id. The sync_state
    // table stores last_sync_job_id as a UUID column, and BullMQ's internal
    // job ID ("1", "2", "3") would fail with "invalid input syntax for type uuid".
    const syncJobId = job.data.syncJobId ?? crypto.randomUUID();
    ...
    try {
      // RLS on ingestion.connectors requires app.tenant_id to be set.
      // Use withTenant when pool is available so the worker can read the row.
      const connector = pool !== undefined
        ? await withTenant(pool, tenantId, (client) => connectorRepo.findById(connectorId, client))
        : await connectorRepo.findById(connectorId);
      if (connector === null) {
        throw new ConnectorNotFoundError(
      ...
      while (hasMore) {
        ...
        // Built-in connectors (connector-csv, oneplatform.csv) are fetched
        // directly without the execution service sandbox.
        let batchResult = await fetchBuiltInCsvBatch(connector.plugin_id, connector.config);

        if (batchResult === null) {
          // Non-built-in: invoke via the Execution Service sandbox.
          const payload = {
          ...
          batchResult = (await fetchResponse.json()) as FetchBatchResponse;
        }
