...
export async function runMigrations(pool: pg.Pool): Promise<MigrationResult> {
  // Create the plugin schema and migration tracking table — both are idempotent.
  await pool.query(`
    CREATE SCHEMA IF NOT EXISTS plugin;

    CREATE TABLE IF NOT EXISTS plugin.schema_migrations (
  ...
  const appliedResult = await pool.query<{ version: string }>(
    "SELECT version FROM plugin.schema_migrations ORDER BY version"
  );
  ...
  for (const filename of migrationFiles) {
    ...
    try {
      ...
      await client.query(
        "INSERT INTO plugin.schema_migrations (version) VALUES ($1)",
        [version]
      );
