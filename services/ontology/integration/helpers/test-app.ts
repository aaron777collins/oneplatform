...
/**
 * Creates a Level 1 test instance of the ontology service.
 * Background cleanup job is disabled to prevent timer leaks in tests.
 */
export async function buildTestApp() {
  const { app, cleanup } = await createServiceApp({
    ...
    // Any non-empty string satisfies this required field; internal calls from
    // tests do not exercise the execution service path
    executionServiceUrl: "http://localhost:13005",
    // Prevents the cleanup background job from starting and leaking timers
