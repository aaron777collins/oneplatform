...
export async function runMigrations(pool: pg.Pool): Promise<MigrationResult> {
  // Ensure the schema and migrations tracking table exist before attempting
  // to read applied versions. Both DDL statements are idempotent.
  await pool.query(`
  ...
  const appliedResult = await pool.query<{ version: string }>(
    "SELECT version FROM execution.schema_migrations ORDER BY version"
  );
  ...
  for (const filename of migrationFiles) {
    ...
    try {
      ...
      await client.query(
        "INSERT INTO execution.schema_migrations (version) VALUES ($1)",
        [version]
      );
