    ...
    const declarations = [
      {
        ...
        content: [
          ...
          `  /** Returns [value, setValue, { isLoading }]. Value persists per-user per-app in platform storage. */`,
          `  export function useAppStorage<T = unknown>(key: string): [T | null, (value: T) => Promise<void>, { isLoading: boolean }];`,
          ...
          `  export interface QueryResult<T> { data: T[] | null; isLoading: boolean; error: Error | null; refetch: () => void; }`,
