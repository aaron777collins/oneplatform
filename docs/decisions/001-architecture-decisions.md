...
- **Orphaned shadow table cleanup:** the Ontology Service runs a background job every hour that uses a two-tier detection strategy: (1) **Primary:** scan the `shadow_table_registry` table in the `ontology` schema, which tracks all active shadow tables with their migration ID, batch ID, and creation timestamp; orphans with no active migration older than 24 hours are logged and auto-dropped. (2) **Secondary (belt-and-suspenders):** scan `information_schema.tables` for tables matching the `shadow_{entity}_{batch_id}` naming pattern that are NOT in the registry — these represent crash-before-registry-write scenarios. Any unregistered shadow tables older than 48 hours are logged as warnings and auto-dropped. The registry write is always the FIRST operation in a batch (before creating the shadow table), but the secondary scan catches the edge case where the registry write succeeds but the shadow table creation crashes halfway, leaving a partial table.
...
- **Redis outage behavior:** rate limiting falls back to LOCAL in-memory sliding window counters per Gateway instance. The fallback limit per instance is calculated as `floor(normal_limit / replica_count)` where `replica_count` is determined at runtime: the Gateway reads its replica count from a lightweight Redis key (`gateway:replicas`) that each Gateway instance increments on startup and decrements on graceful shutdown. During a Redis outage, the last-known replica count is cached in memory. If no cached value exists (first startup with Redis down), it falls back to `OP_GATEWAY_REPLICAS` env var (default 1). This ensures the formula stays accurate even during autoscaling — the env var is only the fallback of last resort, not the primary source. When Redis reconnects, counters are re-synced and the per-replica calculation is no longer needed.
