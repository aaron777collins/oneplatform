...
- **Redis outage behavior:** rate limiting falls back to LOCAL in-memory sliding window counters per Gateway instance. The fallback limit per instance is calculated as `floor(normal_limit / replica_count)` where `replica_count` is read from `OP_GATEWAY_REPLICAS` env var (default 1). This ensures that even with N replicas each applying their own local limit, the aggregate limit never exceeds the normal rate. For example, with a normal limit of 1000 req/min and 3 replicas, each replica allows 333 req/min — the worst case (client hits all replicas evenly) is exactly the normal limit, not 150% of it. When Redis reconnects, counters are re-synced and the per-replica calculation is no longer needed.
