...
> **Memory note:** The limits in `docker-compose.yml` are upper bounds per container.
> Typical idle usage across all containers is around 3–4 GB. The 8 GB minimum ensures
> headroom for peak load and avoids OOM kills during ingestion or migration spikes.
...
> **Note:** All traffic goes through Caddy (ports 80/443). Caddy generates a
> self-signed certificate for `localhost` in development — your browser will show
> a certificate warning on first visit. Accept it to proceed. The API gateway and
> frontend are both reachable at `https://localhost` — Caddy routes `/api/*` to
> the gateway and everything else to the frontend.
...
|---|---|---|
| `OP_BASE_URL` | `https://localhost` | Public-facing URL including protocol. Must match the address users type in their browser. Used in redirect URIs, email links, and CORS. In development with the self-signed certificate this is `https://localhost`. |
| `OP_ALLOWED_ORIGINS` | `https://localhost` | Comma-separated list of allowed CORS origins. In production set this to your exact frontend URL. |
...
**Inter-service authentication** uses Ed25519 key pairs generated automatically
by `op-init` on first boot. Each service receives a unique private key at
`/data/init/keys/<service-name>/private.pem` and verifies peers using public
keys on the shared `init-data` volume. There is no shared secret to set — back
up the `init-data` volume to preserve the generated key material.
...
- [ ] Back up the `init-data` Docker volume — it contains the Ed25519 private keys and master encryption key generated by `op-init`. These cannot be recovered if lost.
...
The browser cannot reach the platform.

1. Check that Docker is running: `docker ps`
2. Check all containers are up: `docker compose -f docker/docker-compose.yml ps`
3. Check Caddy is healthy and test the public endpoint: `curl -sk https://localhost/healthz`
4. Check gateway logs: `docker compose -f docker/docker-compose.yml logs gateway`

> **Browser certificate warning**: On first visit to `https://localhost`, the browser
> will warn about the self-signed certificate. This is expected — click "Advanced"
> and "Proceed" to continue.
...
- **Port conflict**: Another service is using port 80 or 443 (Caddy). Free the conflicting port or adjust docker-compose port mappings.
- **Database connection error**: Check `POSTGRES_PASSWORD` matches in all services.
- **Insufficient memory**: Increase Docker's memory limit to at least 8 GB.
